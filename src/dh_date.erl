...
do_parse(Date, Now, Opts) ->
    Tokens = tokenise(string:to_upper(Date), []),
    case parse(Tokens, Now, Opts) of
        {error, bad_date} ->
	    {error, bad_date};
        {D1, T1} = {{Y, M, D}, {H, M1, S}}
        when is_number(Y),  is_number(M),
            ...
            case calendar:valid_date(D1) of
                true  -> {D1, T1};
                false -> RevDate = reverse(D1),
			 case calendar:valid_date(RevDate) of
			     true  -> {RevDate, T1};
			     false -> {error, bad_date}
			 end
            end;
        _Other -> {error, bad_date}
    end.
...
%% 24 hour clock
parse([Day,X,Month,X,Year,Hour,$:,Min], _Date, _Opts) when ?is_sep(X) ->
    {{Year, Month, Day}, {Hour, Min, 0}};
parse([Day,X,Month,X,Year,Hour,$:,Min,$:,Sec], _Now, _Opts) when ?is_sep(X) ->
...
tokenise([$, | Rest], Acc)     -> tokenise(Rest, Acc);
tokenise([$. | Rest], Acc)     -> tokenise(Rest, [$.] ++ Acc);
tokenise([?SPACE | Rest], Acc) -> tokenise(Rest, Acc);
